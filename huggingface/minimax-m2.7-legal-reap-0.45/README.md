...
MiniMax-M2.7 Legal REAP 0.45 is a more aggressive PLI Labs benchmark-backed
candidate for legal-domain expert pruning. It tests whether legal drafting,
factual synthesis, source-grounded analysis, refusal behavior, and
workflow-sensitive reasoning survive a deeper compression lane.
...
locally evaluable model workflows. This candidate is useful because smaller
local serving only matters if legal capability survives.

The public claim is deliberately narrow:

- this is an aggressive legal-domain REAP candidate;
...
- claims that aggressive pruning is safe for every legal task.

## Evaluation Status

Status: benchmark-backed candidate.

PLI Labs internal legal evaluation indicates promising behavior despite the
deeper compression lane. Public aggregate score tables are still pending.
Until those tables are published, treat this model as a candidate for research
and internal evaluation rather than a validated production legal model.
...
- GitHub model card: https://github.com/ProprietaryLegal/pli-labs/blob/main/reaps/minimax-m2.7-legal-reap-0.45/MODEL_CARD.md
